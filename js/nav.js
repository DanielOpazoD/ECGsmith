...
(function () {
  ...
  // Calcular prefijo relativo al root del proyecto
  const path = window.location.pathname;
  let prefix = '';
  if (path.includes('/capitulos/') || path.includes('/apendices/')) prefix = '../';
  ...
  function score(entry, q) {
    ...
    for (const s of entry.sections) {
      if (norm(s).includes(nQ)) {
        best = Math.max(best, 80);
        if (!sectionMatch) sectionMatch = s;
      }
    }
  ...
  function renderResults(q) {
    ...
    currentMatches.forEach((m, i) => {
      ...
      const url = prefix + e.url;
      const titleH = highlight(e.title, q);
      const sectionH = m.sectionMatch ? highlight(m.sectionMatch, q) : '';
  ...
  function activate(idx) {
    ...
    if (!m) return;
    window.location = prefix + m.entry.url;
  }
